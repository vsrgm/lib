...
// Hardware Mapping
#define RELAY_PIN 0      // GPIO 0
#define FLOAT_SWITCH_PIN 3 // GPIO 3 (RX Pin)
...
void addWebLog(String msg) {
  ...
  if (webLogs.length() > 300) webLogs = webLogs.substring(0, 300);
}
...
void setPump(bool on) {
    if (on && !waterLevelOk && !manualOverride) {
...
void reconnectMqtt() {
  ...
  if (millis() - lastReconnectAttempt > 5000) {
    ...
    if (mqttClient.connect(mqttClientId.c_str())) {
      ...
      mqttCallback((char*)"internal", (byte*)"DISCOVER", 8);
    }
  }
}

void setup() {
  // Use Serial1 if available for debug, or just rely on web logs if using RX pin
  Serial.begin(115200);
  ...
  server.on("/", []() {
    ...
    html += "<h3>RO Pump Monitor</h3>";
    html += "<p>Pump: " + String(pumpActive ? "ON" : "OFF") + "</p>";
    html += "<p>Level: " + String(waterLevelOk ? "OK" : "LOW") + "</p>";
    html += "<p>Manual: " + String(manualOverride ? "ON" : "OFF") + "</p>";
    html += "<div style='font-family:monospace;'>" + webLogs + "</div>";
    html += "<hr><a href='/update'>Update</a></body></html>";
...
void loop() {
  ...
  if (millis() - lastSensorRead > 1000) {
    ...
    // Read float switch (Active LOW usually, so LOW = water presence/YES)
    // User said "YES" -> I'll assume YES means it's okay to pump.
    ...
    if (currentLevel != waterLevelOk) {
        ...
        // Auto-stop if level goes LOW and not in manual override
        if (!waterLevelOk && pumpActive && !manualOverride) {
            setPump(false);
