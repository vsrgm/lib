...
void *start_manual_data_pump(void *arg) {
    ...
    while (!stop_requested) {
        ...
        if (sock >= 0) {
            ...
            static long last_total_bytes = 0;
            if (time(NULL) > last_stats_sent) {
                long current_rate_kbps = (total_bytes_sent - last_total_bytes) * 8 / 1024;
                char s_pkt[128]; snprintf(s_pkt, sizeof(s_pkt), "SENT:%ld|RATE:%ld", total_bytes_sent, current_rate_kbps);
                sendto(stats_sock, s_pkt, strlen(s_pkt), 0, (struct sockaddr *)&stats_addr, sizeof(stats_addr));
                last_stats_sent = time(NULL);
                last_total_bytes = total_bytes_sent;
            }
        }
        xioctl(video_fd, VIDIOC_QBUF, &buf);
    }
...
void *firebase_live_stream_thread(void *arg) {
    ...
    while (1) {
        if (fb_stream_active && strlen(fb_url) > 0) {
            ...
            free(b64_data);
            usleep(100000); // 100ms sleep (max 10 FPS)
        } else {
            sleep(1);
        }
    }
